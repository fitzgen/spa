...
impl<'a> ConstrainTypes<'a> for ast::Expression<'a> {
        ...
        F: FnMut(TypeConstraint),
    {
        ...
        match *self {
            ...
            ast::Expression::Input | ast::Expression::Integer(_) => {
            ...
            ast::Expression::Deref(ref expr) => {
                ...
                f(TypeConstraint::new(
                    pointer_ty,
                    ctx.type_var_for_node(&**expr),
                ));
            ...
            ast::Expression::Greater(ref exprs) => {
                f(TypeConstraint::new(self_ty_var, ctx.int_type()));
                f(TypeConstraint::new(
                    ctx.type_var_for_node(&exprs[0]),
                    ctx.int_type(),
                ));
                f(TypeConstraint::new(
                    ctx.type_var_for_node(&exprs[1]),
                    ctx.int_type(),
                ));
        ...
        F: FnMut(TypeConstraint),
    {
        match *self {
            ...
            ast::Statement::If(ref condition, ref consequent, ref alternative) => {
                f(TypeConstraint::new(
                    ctx.type_var_for_node(condition),
                    ctx.int_type(),
                ));
...
impl<'a> ConstrainTypes<'a> for ast::Function<'a> {
        ...
        F: FnMut(TypeConstraint),
    {
        ...
        f(TypeConstraint::new(
            ctx.type_var_for_node(self.name),
            func_ty,
        ));
...
mod tests {
        ...
        C: AsRef<[&'static str]>,
    {
        ...
        program.constrain_types(&mut ctx, &mut |constraint| {
            actual_constraints.push(constraint);
        });

        let actual_constraints: Vec<_> = actual_constraints
            .into_iter()
            .map(|c| {
